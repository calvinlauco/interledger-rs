        ...
        + 'static,
{
    ...
    // Returns Result<A::AccountId, ApiError>, Err if not authorized or account was not found,
    // Ok if authorized, not to cause any rejections here.
        ...
        .and_then(
            move |path_username: Username,
                ...
                store
                    ...
                    .then(move |account_id: Result<A::AccountId, _>| {
                        if account_id.is_err() {
                            return Either::A(ok(Err(ApiError::account_not_found())));
                        }
                        let account_id = account_id.unwrap();
                        if auth_string == admin_auth_header {
                            return Either::A(ok(Ok(account_id)));
                        }
                        let auth = match AuthToken::from_str(&auth_string) {
                            Ok(auth) => auth,
                            Err(_) => return Either::A(ok(Err(ApiError::account_not_found()))),
                        };
                        Either::B(
                            store
                    ...
                    .map_err::<_, Rejection>(move |()| ApiError::internal_server_error().into())
            },
        )
        .boxed();

    // Receives Result<A::AccountId, ApiError> and rejects if Err if given.
    let admin_or_authorized_user_only =
        |id: Result<A::AccountId, ApiError>| -> Result<_, Rejection> {
            match id {
                Ok(id) => Ok(id),
    ...
    let is_authorized_user_with_account = account_username
        ...
        .and(with_store.clone())
        .and_then(
            move |path_username: Username, auth_string: String, store: S| {
                let auth = match AuthToken::from_str(&auth_string) {
                    Ok(auth) => auth,
                    Err(_) => return Either::A(ok(Err(ApiError::account_not_found()))),
                };
                Either::B(
                    store
                        ...
                        .then(move |authorized_account: Result<A, _>| {
                            ...
                            let authorized_account = authorized_account.unwrap();
                            if &path_username == authorized_account.username() {
                                ok(Ok(authorized_account))
                            } else {
                                ok(Err(ApiError::unauthorized()))
                            }
                        })
                        .map_err::<_, Rejection>(move |()| {
    ...
    let authorized_user_only = |account: Result<A, ApiError>| -> Result<_, Rejection> {
        match account {
    ...
    let get_account = warp::get2()
        ...
        .and(warp::path::end())
        .and_then(admin_or_authorized_user_only)
        .and(with_store.clone())
    ...
    let incoming_payment_notifications = is_admin_or_authorized_user_with_id
        .clone()
