...
use warp::{self, reject::custom, Filter, Rejection};
        ...
        + 'static,
{
        ...
        .and_then(|auth: AuthToken, store: S| {
            store
                ...
                .map_err(move |_| {
                    ...
                    custom(ApiError::default_unauthorized())
                })
        })
        .boxed();
    ...
    let get_accounts = warp::get2()
        ...
        .and_then(|store: S| {
            store
                .get_all_accounts()
                .map_err(|_| custom(ApiError::default_internal_server_error()))
        ...
        .and_then(
            move |id: A::AccountId, account_details: AccountDetails, store: S| {
                ...
                store
                    ...
                    .map_err(move |_| ApiError::default_internal_server_error().into())
                    .and_then(move |account| {
                        connect_to_external_services(handler, account, store_clone, btp)
                    })
                    .and_then(|account: A| Ok(warp::reply::json(&account)))
    ...
    let get_account = warp::get2()
        ...
        .and_then(|id: A::AccountId, store: S| {
            store
                ...
                .map_err(|_| custom(ApiError::account_not_found(None)))
        ...
        .and_then(|id: A::AccountId, settings: AccountSettings, store: S| {
            store
                ...
                .map_err(move |_| {
                    ...
                    Into::<Rejection>::into(ApiError::default_internal_server_error())
                })
    ...
    let get_spsp_well_known = warp::get2()
        ...
        .and_then(move |store: S| {
            ...
            if let Some(username) = default_spsp_account.clone() {
                ...
                Either::A(
                    store
                        ...
                        .and_then(move |id| {
                            ...
                            store
                                .get_accounts(vec![id])
                                .map_err(|_| ApiError::default_internal_server_error().into())
                ...
                Either::B(err(ApiError::default_not_found().into()))
            }
        })
        .boxed();
...
impl ApiError {
    fn account_not_found(not_found_account: Option<&str>) -> Self {
        ApiError::not_found(
            ApiErrorType::Custom("accounts/account-not-found"),
            "Account Not Found",
            Some(match not_found_account {
                Some(account) => format!("{} was not found.", account),
                None => "Username was not found.".to_owned(),
            }),
            None,
            None,
        )
